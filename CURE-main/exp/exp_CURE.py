...
class ExpCURE(Exp):
    def __init__(self, args):
        ...
        if unlearning_method == 'CURE':
            t, f1, self.unlearned_model = self.unlearn_push_most_related_with_random_pull_kl_keep()
    ...
    def unlearn_push_most_related_with_random_pull_kl_keep(self):
        ...
        rel_map, _ = self._select_most_related_3hop(forget_nodes, top_m=5)

        rel_all = self._select_kl_nodes_by_ppr_teleport_decay_near(forget_nodes=forget_nodes,L=3,ratio=0.8,teleport=0.15,alpha=0.85,)
        ...
        for _ in range(epochs):
            ...
            if forget_nodes.numel() > 0:
                ...
                if K > 0:
                    ...
                    loss_push = self._sage_push_neg_only(anchor_emb, neg_emb, neg_mask=neg_mask)
                else:
                    loss_push = torch.tensor(0.0, device=device)
            else:
                loss_push = torch.tensor(0.0, device=device)
            ...
            if forget_nodes.numel() > 0 and len(diff_label_map) > 0:
                ...
                loss_pull_rand = self._sage_pull_pos_only(anchor_emb, rand_emb)
            else:
        ...
        final_f1 = self._calculate_f1_score_dlfd(student, self.dataset_unlearned_test_loader)['F1']
    ...
    def _select_most_related_3hop(self,forget_nodes: torch.Tensor,top_m: int = 5,min_k: int = 1,max_k: int = None,):
    ...
    def _select_kl_nodes_by_ppr_teleport_decay_near(self,forget_nodes: torch.Tensor,L: int,ratio: float,teleport: float = 0.15,alpha: float = 0.85,):
        device = self.device
    ...
    def _sage_push_neg_only(self,anchor_emb: torch.Tensor,neg_emb: torch.Tensor,neg_mask: torch.Tensor = None) -> torch.Tensor:
        import torch.nn.functional as F
        if neg_emb.dim() == 2:
    ...
    def _sage_pull_pos_only(self,anchor_emb: torch.Tensor,pos_emb: torch.Tensor,pos_mask: torch.Tensor = None) -> torch.Tensor:
        import torch.nn.functional as F
        if pos_emb.dim() == 2:
    ...
    def _calculate_f1_score_dlfd(self, model, loader):
        model.eval()
